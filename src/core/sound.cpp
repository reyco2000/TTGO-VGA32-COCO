...
// Master volume, 0-100%. Applied only to the signal deviation from each
// source's calibrated DC offset (see sound_update) — never to the offset
// itself, so silence stays at the same DAC level regardless of volume.
...
static void sound_update(void) {
    ...
    int deviation = (int)((level * source_gain[source][sindex]) / 63u);
    deviation = (deviation * master_volume) / 100;
    int out = deviation + (int)source_offset[source][sindex];
    if (out > 255) out = 255;
    if (out < 0)   out = 0;
...
void sound_reset(void) {
    ...
    // master_volume is intentionally NOT reset here — a machine reset
    // (F4) shouldn't wipe the user's chosen volume level.
    sound_update();
}
...
void sound_volume_up(void) {
    sound_set_volume((master_volume <= 90) ? (uint8_t)(master_volume + 10) : 100);
}

void sound_volume_down(void) {
    sound_set_volume((master_volume >= 10) ? (uint8_t)(master_volume - 10) : 0);
}
